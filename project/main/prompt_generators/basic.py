...
# 2. Define a list of ALL possible keys (including conditionals)
all_keys = [
...
for key in all_keys:
    val = data.get(key)
    # If the value is missing (None), an empty string (""), or an empty list ([]), use "N/A"
    if val in [None, "", []]:
        safe_data[key] = "N/A"
    else:
        safe_data[key] = val

# 4. Format lists into readable strings (for 'verify' and 'output' arrays)
if isinstance(safe_data['verify'], list):
    safe_data['verify'] = ", ".join(safe_data['verify'])
if isinstance(safe_data['output'], list):
    safe_data['output'] = ", ".join(safe_data['output'])

# 5. --- Calculate AND Quantize FIR Coefficients using SciPy ---
def parse_float(val, default=None):
    """Helper to safely parse floats, avoiding 'N/A' crash"""
    try:
        return float(val)
...
# 6. Read the Basic prompt template from rtl_basic_prompt.md
md_template_path = os.path.join(script_dir, '..', '..', 'data', 'md', 'templates', 'rtl_basic_prompt.md')
...
try:
    ...
    print(f"Error: Prompt template 'rtl_basic_prompt.md' not found at:\n{md_template_path}")
    exit(1)

# 7. Format the template with the safe JSON data
final_prompt = prompt_template.format(**safe_data)

# 8. Write the generated prompt to rtl_agent_prompt.md
# Build the path to project/data/md
output_dir = os.path.join(script_dir, '..', '..', 'data', 'md', 'agent_prompts')
output_dir = os.path.normpath(output_dir)

# Create the 'md' folder if it doesn't already exist
