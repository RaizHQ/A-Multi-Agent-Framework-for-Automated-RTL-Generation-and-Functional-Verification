...
# 3. Fill missing or empty keys with "N/A"
safe_data = {}
for key in all_keys:
    val = data.get(key)
    if val in [None, "", []]:
...
# 6. Read the Advanced prompt template from rtl_advance_prompt.md
md_template_path = os.path.join(script_dir, '..', '..', 'data', 'md', 'templates', 'rtl_advance_prompt.md')
...
try:
    ...
    print(f"Error: Prompt template 'rtl_advance_prompt.md' not found at:\n{md_template_path}")
    exit(1)
...
print(f"Successfully generated the prompt and saved it to {output_filename}")